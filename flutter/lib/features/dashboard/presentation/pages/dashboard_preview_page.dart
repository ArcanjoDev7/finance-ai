...
class CryptoItem {
  const CryptoItem({required this.asset, required this.amount, required this.operation});
  final String asset;
  final double amount;
...
class _DashboardPreviewPageState extends State<DashboardPreviewPage> {
  ...
  void _addCrypto(CryptoItem item) => setState(() => _cryptos.insert(0, item));
  ...
  Future<void> _showCryptoForm() async {
    final item = await showModalBottomSheet<CryptoItem>(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => const _CryptoForm());
    if (item == null || !mounted) return;
    _addCrypto(item);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Operação de cripto adicionada')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final content = switch (_page) {
      ...
      FinancePage.crypto => _CryptoPage(items: _cryptos, hideValues: _hideValues, onAdd: _showCryptoForm),
      FinancePage.goals => _GoalsPage(items: _investments, hideValues: _hideValues),
      FinancePage.cards => _CardsPage(entries: _entries, hideValues: _hideValues, onAdd: _showEntryForm),
      FinancePage.reports => _ReportsPage(entries: _entries, hideValues: _hideValues),
      FinancePage.assistant => ChatPage(onTransactionCreated: _addEntry, onInvestmentCreated: _addInvestment, onCryptoCreated: _addCrypto),
      _ => _EmptyFeaturePage(page: _page, onNavigate: _go, onAddEntry: _showEntryForm, onAddInvestment: _showInvestmentForm),
    };
    return Scaffold(
  ...
  @override Widget build(BuildContext context) => AppBar(
    ...
    actions: [
      if (MediaQuery.sizeOf(context).width >= AppBreakpoints.expanded)
        SizedBox(width: 280, child: TextField(decoration: const InputDecoration(isDense: true, prefixIcon: Icon(Icons.search), hintText: 'Buscar movimentações'))),
      const SizedBox(width: 8),
      IconButton(tooltip: 'Notificações', icon: const Badge(smallSize: 7, child: Icon(Icons.notifications_none_rounded)), onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Você está em dia. Sem novas notificações.')))),
...
class _DashboardContent extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(28), children: [
      ...
      _Panel(title: 'Visão do período', action: TextButton(onPressed: () => onNavigate(FinancePage.reports), child: const Text('Relatório completo')), child: LayoutBuilder(builder: (context, box) => box.maxWidth >= 860 ? Row(children: [Expanded(flex: 3, child: _CashflowChart(income: income, expense: expense)), const SizedBox(width: 24), Expanded(flex: 2, child: _AllocationSummary(investments: investmentsTotal, available: income - expense, hideValues: hideValues))]) : Column(children: [_CashflowChart(income: income, expense: expense), const SizedBox(height: 24), _AllocationSummary(investments: investmentsTotal, available: income - expense, hideValues: hideValues)]))),
      const SizedBox(height: 18),
...
class _EmptyFeaturePage extends StatelessWidget { const _EmptyFeaturePage({required this.page, required this.onNavigate, required this.onAddEntry, required this.onAddInvestment}); final FinancePage page; final ValueChanged<FinancePage> onNavigate; final Future<void> Function({EntryKind kind}) onAddEntry; final Future<void> Function() onAddInvestment; @override Widget build(BuildContext context) { final data = switch(page) { FinancePage.crypto => (Icons.currency_bitcoin, 'Criptomoedas', 'Registre uma compra de cripto e acompanhe sua carteira.', 'Adicionar compra', () => onAddInvestment()), FinancePage.cards => (Icons.credit_card_outlined, 'Cartões', 'Registre despesas do cartão e acompanhe a fatura.', 'Adicionar despesa', () => onAddEntry(kind: EntryKind.expense)), FinancePage.goals => (Icons.flag_outlined, 'Metas', 'Registre receitas e despesas para começar sua primeira meta.', 'Ver movimentações', () => onNavigate(FinancePage.transactions)), FinancePage.reports => (Icons.insights_outlined, 'Relatórios', 'Os relatórios aparecem a partir das suas movimentações.', 'Ver movimentações', () => onNavigate(FinancePage.transactions)), _ => (Icons.settings_outlined, 'Configurações', 'Preferências de conta e privacidade serão sincronizadas.', 'Abrir assistente', () => onNavigate(FinancePage.assistant)), }; return Center(child: _Panel(title: data.$2, child: _HelpfulEmpty(icon: data.$1, title: data.$2, text: data.$3, action: FilledButton.icon(onPressed: data.$5, icon: const Icon(Icons.add), label: Text(data.$4))))); } }

class _CryptoPage extends StatelessWidget { const _CryptoPage({required this.items, required this.hideValues, required this.onAdd}); final List<CryptoItem> items; final bool hideValues; final Future<void> Function() onAdd; @override Widget build(BuildContext context) { final total = items.fold(0.0, (sum, item) => sum + (item.operation == 'Venda' ? -item.amount : item.amount)); return ListView(padding: const EdgeInsets.all(28), children: [_PageHeader(title: 'Criptomoedas', subtitle: 'Registre compras, vendas e conversões com o assistente ou manualmente.', action: FilledButton.icon(onPressed: onAdd, icon: const Icon(Icons.add), label: const Text('Nova operação'))), const SizedBox(height: 22), _HeroCard(value: total, hideValues: hideValues, label: 'Alocado em cripto', onAssistant: onAdd), const SizedBox(height: 18), _Panel(title: 'Operações recentes', child: items.isEmpty ? _HelpfulEmpty(icon: Icons.currency_bitcoin, title: 'Sua carteira cripto está vazia', text: 'Diga “comprei 5.000 reais de Bitcoin” ao assistente ou registre uma operação.', action: FilledButton(onPressed: onAdd, child: const Text('Adicionar operação'))) : Column(children: items.map((item) => ListTile(leading: CircleAvatar(backgroundColor: const Color(0xFF5A3D12), child: const Icon(Icons.currency_bitcoin)), title: Text(item.asset), subtitle: Text(item.operation), trailing: _FinancialValue(value: item.amount, hidden: hideValues, negative: item.operation == 'Venda', style: Theme.of(context).textTheme.titleMedium))).toList()))]); } }

class _GoalsPage extends StatelessWidget { const _GoalsPage({required this.items, required this.hideValues}); final List<InvestmentItem> items; final bool hideValues; @override Widget build(BuildContext context) { final fixed = items.where((item) => const ['CDB', 'LCI', 'LCA', 'Tesouro'].contains(item.type)).toList(); final total = fixed.fold(0.0, (sum, item) => sum + item.amount); return ListView(padding: const EdgeInsets.all(28), children: [_PageHeader(title: 'Metas', subtitle: 'Acompanhe somente sua reserva e renda fixa.'), const SizedBox(height: 22), _Panel(title: 'Progresso da renda fixa', child: fixed.isEmpty ? const _HelpfulEmpty(icon: Icons.flag_outlined, title: 'Nenhuma meta de renda fixa ainda', text: 'CDB, LCI, LCA e Tesouro aparecem aqui. Ações e criptomoedas ficam fora desta visão.') : Center(child: SizedBox(width: 230, height: 230, child: Stack(alignment: Alignment.center, children: [CircularProgressIndicator(value: 0.72, strokeWidth: 18, backgroundColor: const Color(0xFF342E3D), color: AppColors.brand), Column(mainAxisSize: MainAxisSize.min, children: [const Text('Renda fixa'), _FinancialValue(value: total, hidden: hideValues, style: Theme.of(context).textTheme.titleLarge)])])))), const SizedBox(height: 18), _Panel(title: 'Composição', child: fixed.isEmpty ? const Text('Adicione um investimento de renda fixa para iniciar.') : Column(children: fixed.map((item) => ListTile(title: Text(item.name), subtitle: Text(item.type), trailing: _FinancialValue(value: item.amount, hidden: hideValues, style: Theme.of(context).textTheme.titleMedium))).toList()))]); } }

class _CardsPage extends StatelessWidget { const _CardsPage({required this.entries, required this.hideValues, required this.onAdd}); final List<FinanceEntry> entries; final bool hideValues; final Future<void> Function({EntryKind kind}) onAdd; @override Widget build(BuildContext context) { final invoice = entries.where((item) => item.kind == EntryKind.expense).fold(0.0, (sum, item) => sum + item.amount); const limit = 5000.0; return ListView(padding: const EdgeInsets.all(28), children: [_PageHeader(title: 'Cartões', subtitle: 'Veja a fatura atual e registre despesas do cartão.', action: FilledButton.icon(onPressed: () => onAdd(kind: EntryKind.expense), icon: const Icon(Icons.add), label: const Text('Adicionar lançamento'))), const SizedBox(height: 22), _Panel(title: 'Fatura atual', child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_FinancialValue(value: invoice, hidden: hideValues, style: Theme.of(context).textTheme.displaySmall), const SizedBox(height: 12), LinearProgressIndicator(value: (invoice / limit).clamp(0.0, 1.0)), const SizedBox(height: 8), const Text('Limite de referência: R\$ 5.000,00', style: TextStyle(color: Colors.white70))])), const SizedBox(height: 18), _Panel(title: 'Lançamentos', child: entries.where((item) => item.kind == EntryKind.expense).isEmpty ? _HelpfulEmpty(icon: Icons.credit_card_outlined, title: 'Nenhum lançamento na fatura', text: 'Adicione uma despesa para simular sua fatura atual.', action: FilledButton(onPressed: () => onAdd(kind: EntryKind.expense), child: const Text('Adicionar despesa'))) : Column(children: entries.where((item) => item.kind == EntryKind.expense).map((item) => _EntryRow(entry: item, hideValues: hideValues)).toList()))]); } }

class _ReportsPage extends StatelessWidget { const _ReportsPage({required this.entries, required this.hideValues}); final List<FinanceEntry> entries; final bool hideValues; @override Widget build(BuildContext context) { final income = entries.where((item) => item.kind == EntryKind.income).fold(0.0, (sum, item) => sum + item.amount); final expense = entries.where((item) => item.kind == EntryKind.expense).fold(0.0, (sum, item) => sum + item.amount); return ListView(padding: const EdgeInsets.all(28), children: [_PageHeader(title: 'Relatórios', subtitle: 'Resumo financeiro do período atual.'), const SizedBox(height: 22), _Panel(title: 'Fluxo mensal', child: entries.isEmpty ? const _HelpfulEmpty(icon: Icons.insights_outlined, title: 'Ainda não há dados suficientes', text: 'Adicione receitas ou despesas para gerar seus relatórios.') : Column(children: [_ReportRow(label: 'Receitas', value: income, hideValues: hideValues, color: AppColors.positive), _ReportRow(label: 'Despesas', value: expense, hideValues: hideValues, color: AppColors.negative), const Divider(), _ReportRow(label: 'Resultado', value: income - expense, hideValues: hideValues, color: AppColors.brand)]))]); } }
class _ReportRow extends StatelessWidget { const _ReportRow({required this.label, required this.value, required this.hideValues, required this.color}); final String label; final double value; final bool hideValues; final Color color; @override Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 10), child: Row(children: [Expanded(child: Text(label)), SizedBox(width: 130, child: LinearProgressIndicator(value: value <= 0 ? 0 : (value / 10000).clamp(0.0, 1.0), color: color, backgroundColor: const Color(0xFF342E3D))), const SizedBox(width: 16), _FinancialValue(value: value, hidden: hideValues, style: Theme.of(context).textTheme.titleMedium)])); }

class _CashflowChart extends StatelessWidget { const _CashflowChart({required this.income, required this.expense}); final double income; final double expense; @override Widget build(BuildContext context) { final max = [income, expense, 1.0].reduce((a, b) => a > b ? a : b); return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Entradas e saídas', style: TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 18), SizedBox(height: 150, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [Expanded(child: _ChartBar(label: 'Receitas', value: income, height: income / max, color: AppColors.positive)), const SizedBox(width: 24), Expanded(child: _ChartBar(label: 'Despesas', value: expense, height: expense / max, color: AppColors.negative)), const SizedBox(width: 24), Expanded(child: _ChartBar(label: 'Resultado', value: income - expense, height: (income - expense).abs() / max, color: AppColors.brand))]))]); } }
class _ChartBar extends StatelessWidget { const _ChartBar({required this.label, required this.value, required this.height, required this.color}); final String label; final double value; final double height; final Color color; @override Widget build(BuildContext context) => Column(mainAxisAlignment: MainAxisAlignment.end, children: [Expanded(child: Align(alignment: Alignment.bottomCenter, child: Tooltip(message: 'R\$ ${value.toStringAsFixed(2)}', child: AnimatedContainer(duration: AppAnimations.standard, width: 48, height: (118 * height.clamp(0.06, 1.0)).toDouble(), decoration: BoxDecoration(color: color, borderRadius: const BorderRadius.vertical(top: Radius.circular(10))))))), const SizedBox(height: 8), Text(label, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary), overflow: TextOverflow.ellipsis)]); }
class _AllocationSummary extends StatelessWidget { const _AllocationSummary({required this.investments, required this.available, required this.hideValues}); final double investments; final double available; final bool hideValues; @override Widget build(BuildContext context) { final total = investments + available; final ratio = total <= 0 ? 0.0 : investments / total; return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Distribuição', style: TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 16), Row(children: [SizedBox(width: 76, height: 76, child: Stack(alignment: Alignment.center, children: [CircularProgressIndicator(value: ratio, strokeWidth: 9, color: AppColors.info, backgroundColor: AppColors.border), Text('${(ratio * 100).round()}%', style: const TextStyle(fontWeight: FontWeight.w700))])), const SizedBox(width: 18), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_AllocationLine(label: 'Disponível', value: available, hidden: hideValues, color: AppColors.brand), const SizedBox(height: 9), _AllocationLine(label: 'Investimentos', value: investments, hidden: hideValues, color: AppColors.info)]))])]); } }
class _AllocationLine extends StatelessWidget { const _AllocationLine({required this.label, required this.value, required this.hidden, required this.color}); final String label; final double value; final bool hidden; final Color color; @override Widget build(BuildContext context) => Row(children: [Container(width: 8, height: 8, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 8), Expanded(child: Text(label, style: const TextStyle(fontSize: 13, color: AppColors.textSecondary))), _FinancialValue(value: value, hidden: hidden, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700))]); }
...
class _CryptoForm extends StatefulWidget { const _CryptoForm(); @override State<_CryptoForm> createState() => _CryptoFormState(); }
class _CryptoFormState extends State<_CryptoForm> { final asset = TextEditingController(text: 'Bitcoin'); final amount = TextEditingController(); String operation = 'Compra'; @override void dispose() { asset.dispose(); amount.dispose(); super.dispose(); } void save() { final value = double.tryParse(amount.text.replaceAll(',', '.')); if (value == null || value <= 0 || asset.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Informe ativo e valor válidos.'))); return; } Navigator.pop(context, CryptoItem(asset: asset.text.trim(), amount: value, operation: operation)); } @override Widget build(BuildContext context) => _FormSheet(title: 'Operação em cripto', child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [SegmentedButton<String>(segments: const [ButtonSegment(value: 'Compra', label: Text('Compra')), ButtonSegment(value: 'Venda', label: Text('Venda')), ButtonSegment(value: 'Conversão', label: Text('Conversão'))], selected: {operation}, onSelectionChanged: (value) => setState(() => operation = value.first)), const SizedBox(height: 16), TextField(controller: asset, decoration: const InputDecoration(labelText: 'Ativo', hintText: 'Ex.: Bitcoin ou Ethereum')), const SizedBox(height: 12), TextField(controller: amount, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Valor da operação', prefixText: 'R\$ ')), const SizedBox(height: 24), FilledButton(onPressed: save, child: const Text('Salvar operação'))])); }
